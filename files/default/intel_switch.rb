...
Ohai.plugin(:IntelSwitch) do
  ...
  def get_attributes( port )
    ...
    attrs_sysfs = %w[ autoneg autoneg_basepage autoneg_link_inhb_timer
                      autoneg_link_inhb_timer_kx bcast_flooding bcast_pruning def_cfi 
                      def_dscp def_pri def_pri2 def_swpri def_vlan def_vlan2 
                      dot1x_state drop_bv drop_tagged drop_untagged eee_mode 
                      eee_tx_activity_timeout eee_tx_lpi_timeout eye_score 
                      fabric_loopback ignore_ifg_errors learning loopback 
                      mac_table_address_aging_time max_frame_size mcast_flooding
                      mcast_pruning parse_mpls parser parser_store_mpls
                      parser_vlan1_tag parser_vlan2_tag pause_mode replace_dscp
                      routed_frame_update_fields rx_class_pause rx_cut_through 
                      rx_priority_map security_action swpri_dscp_pref swpri_source
                      timestamp_generation tx_cut_through ucast_flooding 
                      ucast_pruning update_dscp update_ttl ]
   
    attrs_sd = %w[ Autoneg AutonegBasepage AutonegLinkInhbTimer 
  ...
  def get_qos( port )
    ...
    attrs_sysfs = %w[ auto_pause_mode dscp_swpri_map global_usage priv_wm shared_pri_wm 
                      ...
                      rx_priority_map tx_priority_map sched_group_priset_num sched_groups 
  ...
  # Gets the cpu rate limiting configuration
  # Params:
  # +port+:: the netdev to query
  # Returns:
  # (Mash) the mash of the cpu configuration
  def get_cpu_rate_limit( port )
    ret = Mash.new
      ...
      if File.directory?( "/sys/class/net/" + line.strip + "/switch" )
        if line.strip == "sw0p0"
          ports[ line.strip ] = {
            ...
            :CpuRateLimit => get_cpu_rate_limit( line.strip ),
  ...
  def get_vlans( netdev )
    ...
    so.stdout.lines do |line|
      ...
      if line =~ /^port\s+vlan\s+ids$/
        ...
        if line =~ /\s(\d+)\s+?(PVID )?(Egress Untagged)?/
